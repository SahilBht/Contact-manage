import React from "react";

// Popup component to display contact details
const Popup = ({ close, el }) => {
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-opacity-20 bg-black flex items-center justify-center text-black">
      <div className="bg-white drop-shadow-md rounded-md p-4 w-mid">
        {/* Header section with title and close button */}
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-lg font-medium">Contact Details</h2>
          <button
            className="text-gray-100 px-2 rounded-md bg-pink-500 hover:text-gray-800"
            onClick={() => close()} // Close the popup when button is clicked
          >
            Close
          </button>
        </div>

        {/* Contact details section */}
        <div key={el.id} className="bg-gray-300 m-4 p-4 text-black">
          <div className="w-3/4 m-auto">
            <img
              className="w-full rounded-full"
              src="data:image/png;base64,"
              alt="Contact Avatar" // Displaying a default avatar image
            />
            <div className="p-4"></div>
          </div>

          {/* Displaying contact details */}
          <div className="text-left">
            <p>First Name: {el.first_name}</p>
            <p>Last Name: {el.last_name}</p>
            <p>Mobile: {el.mob}</p>
            <p>Status: {el.status === "active" ? "Active" : "Inactive"}</p>
          </div>
        </div>
      </div>
    </div>
  );
};

// Exporting the Popup component as the default export
export default Popup;
